...
public class Building : UIInteractable
{
    ...
    [SerializeField] private GameObject completedBuilding;
    ...
    private void UpdateInteractions()
    {
        if (!isInteractable)
        {
            isInteractable = CheckRequiredItems();
    ...
    private bool CheckRequiredItems()
    {
        ...
        switch (currentStage)
        {
            case BuildingStage.Foundation:
                ...
                if (cell.numberOfItems >= currentAmountNeeded)
                {
                    return true;
                }
                break;
    ...
    protected override void Activate(string message)
    {
        ...
        switch (action)
        {
            ...
            case InteractionTypes.Add:
                cell.Remove(currentAmountNeeded);
                cell.Clear();
    ...
    private void UpdateAmountNeeded()
    {
        switch (currentStage)
        {
            case BuildingStage.Foundation:
                ...
                cell.LockInventory(foundationRequiredItems[0]);
                break;
            case BuildingStage.Framing:
                currentAmountNeeded = amountOfFramingItems[0];
                cell.LockInventory(framingRequiredItems[0]);
                break;
            default:
                break;
